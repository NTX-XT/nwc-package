...
export class NWCPackageManager {
	...
	private async packageWorkflows(sources: INWCWorkflowSource[], skipExports: boolean = false): Promise<void> {
		...
		for (var i = 0; i < deployOrder.length; i++) {
			...
			const allActions = NWCPackageManager.getFlatActionsArray(workflow!.workflowDefinitionAsObject!.actions) as INWCWorkflowAction[]
			solutionWorkflow.connectionData = {}
			for (const [key, value] of Object.entries(workflow!.workflowDefinitionAsObject!.inUseXtensions)) {
				for (const actionId of value.usedByActionIds) {
					var action = allActions.find(action => action.id === actionId)
					for (const property of action!.configuration.properties) {
						let foundConnection = property.parameters.find(parameter => parameter.name === "['X_NTX_XTENSION_INPUT']")
						if (foundConnection) {
